...
async fn connect_and_run(
    ...
    sync_month(&client, &calendars, &credential, current_month, events).await;
    ...
    loop {
        ...
        match wake {
            Wake::Poll => sync_month(&client, &calendars, &credential, current_month, events).await,
        ...
        for command in woke_on_command.into_iter().chain(std::iter::from_fn(|| commands.try_recv().ok())) {
            match command {
                ...
                CalendarCommand::Refresh => sync_month(&client, &calendars, &credential, current_month, events).await,
...
async fn sync_month(client: &DavClient, calendars: &[CalendarInfo], credential: &Credential, month: NaiveDate, events: &async_channel::Sender<CalendarSessionEvent>) {
