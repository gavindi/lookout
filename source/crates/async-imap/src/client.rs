...
// Lookout's local addition (this crate is vendored and patched - see the
// header comment in Cargo.toml).
impl<T: Read + Write + Unpin + fmt::Debug + Send> Session<T> {
    /// Swaps the underlying stream in place without sending any command.
    ///
    /// This is what lets a client issue `COMPRESS DEFLATE` itself through the
    /// borrowing `run_command_and_check_ok` (so a rejected command degrades
    /// gracefully) and then rewrap the connection for the compressed
    /// transport - upstream's `Session::compress` consumes the session and
    /// loses it on error.
    ///
    /// The caller owns any protocol state the new stream implies: a server
    ...
    pub fn map_stream<S: Read + Write + Unpin + fmt::Debug + Send>(self, f: impl FnOnce(T) -> S) -> Session<S> {
        ...
        let Connection { stream, request_ids } = conn;
        let conn = Connection {
            stream: ImapStream::new(f(stream.into_inner())),
            request_ids,
        };
        Session {
            conn,
