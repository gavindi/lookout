...
impl<T: Read + Write + Unpin + fmt::Debug> DeflateStream<T> {
    // Lookout's local change (this crate is vendored and patched - see the
    // header comment in Cargo.toml): `new` is `pub(crate)` upstream, but the
    // type itself is publicly re-exported and the mail session must construct
    // a `DeflateStream` around its TLS stream after `COMPRESS DEFLATE`
    // succeeds. Widening the constructor to `pub` changes nothing upstream.
    /// Wraps `stream` with DEFLATE compression on both directions.
