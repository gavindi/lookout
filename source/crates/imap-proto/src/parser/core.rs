...
// From RFC5234
//
// Local patch (Lookout): upstream djc/imap-proto's `main` branch relaxes
// this to accept any non-NUL byte, and that fix is unreleased as of 0.16.7.
// Real-world IMAP servers (Dovecot, Cyrus, ...) regularly emit 8-bit bytes
// inside quoted strings and literals - most commonly raw UTF-8 subjects in
// ENVELOPE responses (RFC 6855 / IMAP4rev2 / RFC 9051 formally allow
// UTF-8 there), but also MIME filenames in non-UTF-8 encodings like
// Latin-1. Strict 7-bit rejection fails the parse of a single such mail
// (`TakeWhile1`) and desyncs the entire response stream. Accept any
// non-NUL byte and let the higher-level UTF-8 conversion deal with
// non-UTF-8 sequences via lossy decoding.
