...
Completed 17 Aug 2026 (`9268e0e`): redundant SELECT on IDLE wake, redundant post-sync STATUS, capability caching, SQLite pragmas (mail + DAV), `body_index_text` size-check-first, cached remote-content scan, and the vendored-parser round (except `rfc5464` `check_path`). Remaining: the SQLite N+1 batching, the FTS search-path JOIN, and `check_path`.
