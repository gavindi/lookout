...
- [ ] Kill the redundant SELECT on every IDLE wake — `sync_mailbox` unconditionally re-selects the folder (`session.rs:2113`) even though `session_selected == current_mailbox_id` is guaranteed before IDLE re-entry (`session.rs:674-677`); skip it when the session is already on that mailbox
- [ ] Kill the redundant STATUS after sync — the unread count for the woken folder is computable from the flags already fetched; drop `refresh_one_folder_count`'s extra round trip (`session.rs:728`)
- [ ] Cache capabilities — `session.capabilities()` issues a live `CAPABILITY` command on **every** MOVE batch (`session.rs:1744`, async-imap `client.rs:674-683`); fetch once after login, store `has MOVE` in session state
- [ ] Batch the per-UID SQLite N+1s:
  - `has_body` per UID in the prefetch envelope pass (`session.rs:1577-1579`) → one `SELECT uid FROM bodies WHERE mailbox_id=? AND uidvalidity=?`
  - `delete_message` per UID in move handlers (`session.rs:1113-1117`, `1159-1164`) → `DELETE ... WHERE uid IN (...)`
  - `update_flags`/`update_keywords` per UID per batch (`session.rs:1333`, `1444`) → one `UPDATE ... WHERE uid IN (...)`
  - `snooze_message` per UID (`session.rs:1225-1229`) → one batched UPDATE
- [ ] Fix `replace_messages`' FTS side effect on search results — the search path loads each hit with its own SELECT + JSON parse (`cache.rs:897-906`, up to 300 per keystroke on the UI thread) → single FTS→messages JOIN
- [ ] SQLite pragmas: `synchronous=NORMAL` on the mail cache (disposable data, currently FULL = fsync per autocommit commit) and the DAV cache (which also lacks WAL/busy_timeout, `dav/src/cache.rs:110-127`)
- [ ] `body_index_text` strips the full HTML before the size check (`cache.rs:194-210`) → check `FULL_BODY_INDEX_BYTES` first
- [ ] Cache the remote-content scan per body (`window.rs:13462`) — re-scans the whole HTML on every render/selection
- [ ] Vendored-parser fixes:
  - `section_part` uses `Vec::insert(0, …)` → O(k²) per part path (`imap-proto/src/parser/rfc3501/body.rs:16-20`) → `once(part).chain(rest)`
  - `rfc5464.rs` O(n²) `check_path` rescan per path component (`rfc5464.rs:71-83`) → single linear scan
  - `rfc5464.rs` String churn + `slice_to_str().unwrap()` panics (`rfc5464.rs:124-150`) → `Cow<'a, str>` + `map_res(from_utf8)`
  - `rfc2971.rs`/`rfc4314.rs` use `nom::character::complete::*` in a streaming pipeline (`rfc2971.rs:14-15`, `rfc4314.rs:15-17`) — a response split across TCP segments kills the connection instead of waiting → switch to `streaming` variants
  - Command builder `format!`+`into_bytes()` double work and `to_string()` temps per sequence number (`builders/command.rs:30,214-227`) → single-pass `Vec<u8>` with `itoa`-style formatting
